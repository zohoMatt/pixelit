 ...
 */
const getData = (width, height, xPixels, yPixels, rgbaBuffer, mode = 'around') => {
    const xGap = width / xPixels;
    const xStartPt = mode === 'start' ? 0 : xGap / 2;
    const yGap = height / yPixels;
    const yStartPt = mode === 'start' ? 0 : yGap / 2;

    const matrix = Array(xPixels)
        .fill(0)
        .map(() => Array(yPixels).fill('')); // xPixels * yPixels matrix
 ...
 */
const sampling = async (filePath, xSamples, ySamples, options = { mode: 'around' }) => {
    let image = null;
    try {
